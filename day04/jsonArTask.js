...
p1={name:"사과", price:3000, stock:3};
p2={name:"바나나", price:2500, stock:5};
p3={name:"포도", price:1500, stock:2};
...
let pJSON=JSON.stringify(products);

file.writeFile("day04/shop.json", pJSON, "utf-8", function(e){
    if(e){
        console.log(e)
    }else{
        console.log("출력 성공");
...
file.readFile("day04/shop.json", "utf-8", function(e,datas){
    let productsAr=JSON.parse(pJSON);
    // 총 가격과 총 재고 수를 Object에 담은 뒤 sum.json으로 출력한다.
    productsAr.map(function(v){return  v.price*v.stock}).foreach(function(v){totalP+=v; });
    productsAr.push(totalP);
